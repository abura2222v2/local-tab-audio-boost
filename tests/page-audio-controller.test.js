...
function createFakeMediaElement({ tagName = 'VIDEO', currentSrc = '', crossOrigin = '', mediaKeys = null } = {}) {
  return { tagName, currentSrc, src: '', crossOrigin, mediaKeys, parentElement: null };
}
...
function createHarness({ origin = 'https://example.com', contextState = 'running', withAudioContext = true } = {}) {
  ...
  return {
    ...
    notifySourceChanged: (target) => {
      if (observerCallback) observerCallback([{ type: 'attributes', target }]);
    },
    addSourceDynamically: (media) => {
      const source = { tagName: 'SOURCE', parentElement: media, querySelectorAll: () => [] };
      if (observerCallback) observerCallback([{ type: 'childList', target: media, addedNodes: [source] }]);
    },
...
test('controller: appending a ready source element retries its existing parent media element', () => {
  const h = createHarness();
  const video = createFakeMediaElement();
  h.setMedia([video]);
  h.load();
  ...
  video.currentSrc = 'https://example.com/source-child.mp4';
  h.addSourceDynamically(video);
